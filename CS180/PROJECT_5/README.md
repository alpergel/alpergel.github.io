...
<p style="margin: 0 0 10px; color: #334155;">
To carry out the noise process, we just sample random noise in the same data shape as the mnist data, then clamp the input + noise * sigma to a [0.0,1.0] range to ensure proper normalization/visualization. The figure below visualizes the noising process using sigma values of [0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0]
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 26:</b> Visualization of the noising process for MNIST digits at sigma values [0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0].
</p>
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 27:</b> Samples generated by the UNet denoiser after epoch 1 (left) and after epoch 5 (right).
</p>
After the full training process, we are left with an training loss curve that looks like:
<p style="margin: 32px 0; display: flex; flex-direction: row; justify-content: center;">
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 28:</b> UNet Denoiser training loss curve over epochs.
</p>
...
<p style="margin: 32px 0; display: flex; flex-direction: row; justify-content: center;">
  <img src="assets/SectionB/1.2.2/out-of-dist.png" alt="UNet Out-of-Distribution Noise Visualization" style="width: 70%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
</p>
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 29:</b> Visualization of the UNet denoiser outputs on out-of-distribution noise levels.
</p>

<h3 id="required-part-1">Part B.1.2.3: Denoising Pure Noise</h3>
<p style="margin: 0 0 10px; color: #334155;">
When using the same model for denoising pure noise, we see an interesting relationship emerge where it essentially makes a output that looks like a stack of all 0 through 9 digits, which looks like a blob (as seen in the figures below). Since each digit has equal probability to be formed from the random noise, the model tries to create an amalgamation of all the digits in its training data, which can be seen in the samples visualized below.
<p style="margin: 32px 0; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 24px;">
  <img src="assets/SectionB/1.2.3/epoch_1_sampling.png" alt="Pure Noise Denoising Epoch 1" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
  <img src="assets/SectionB/1.2.3/epoch_5_sampling.png" alt="Pure Noise Denoising Epoch 5" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
</p>
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 30:</b> UNet Denoiser samples from pure noise at the end of epoch 1 (top) and epoch 5 (bottom).
</p>
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 31:</b> Loss curve for pure noise denoising with the UNet model.
</p>
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 32:</b> Loss curve for time-conditioned UNet during training.
</p>

<h3 id="required-part-1">Part B.2.3: Sampling from the UNet</h3>
<p style="margin: 0 0 10px; color: #334155;">
Using the algorithm provided in project spec, the sampling process is fairly straight-forward with the already trained time-conditioned UNet from the previous step. We can see that the results are already QUITE a lot better than the previous UNet sampling visualizations, just from this minor modification of time conditioning. This allows us to sample the time-conditioned UNet at various epochs as visualized below:
<p style="margin: 32px 0; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 24px;">
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 33:</b> Samples generated from the time-conditioned UNet at epoch 1 (top), epoch 5 (middle), and epoch 10 (bottom).
</p>
...
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 34:</b> Loss curve for class-conditioned UNet during training.
</p>
...
<p style="margin: 32px 0; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 24px;">
  <img src="assets/SectionB/2.6/epoch_1.png" alt="Class-Conditioned UNet Sampling Epoch 1" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
  <img src="assets/SectionB/2.6/epoch_5.png" alt="Class-Conditioned UNet Sampling Epoch 5" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
  <img src="assets/SectionB/2.6/epoch_10.png" alt="Class-Conditioned UNet Sampling Epoch 10" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
</p>
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 35:</b> Samples generated from the class-conditioned UNet at epoch 1 (top), epoch 5 (middle), and epoch 10 (bottom).
</p>

Further, I was able to remove the ExponentialLR Scheduler by changing the optimizer from Adam to AdamW, which has been shown to have better convergence for UNet's in general. This results in the same visual quality, while converging relatively quickly as can be seen below:
...
<p style="margin: 32px 0; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 24px;">
  <img src="assets/SectionB/2.6/epoch_5_without_scheduler.png" alt="Class-Conditioned UNet Sampling Epoch 5" style="width: 45%; border-radius: 10px; border: 1.5px solid #e5e7eb;">
</p>
<p style="text-align: center; color: #64748b; font-size: 1.08rem;">
  <b>Figure 35:</b> Samples generated from the class-conditioned UNet trained without the scheduler at epoch 5.
</p>